    ...
    static class CreateSosAndDefFIles
    {
        ...
        public static async Task<bool> CreateSosFile(string newfile)
        {
            ...
            try
            {
                ...
                {
                    var fileBytes = (_rHelper.ExamType=="NCEE" || _rHelper.ExamType == "GIFTED") ? await Repository.GetFile("sos", _rHelper.Paper) : await Repository.GetFile("sos", _rHelper.Job);
        ...
        public static async Task<bool> CreateDefFile(string newfile)
        {
            try
            {
                if (!File.Exists(newfile))
                {
                    var fileBytes = (_rHelper.ExamType=="NCEE" || _rHelper.ExamType == "GIFTED") ? await Repository.GetFile("def", _rHelper.Paper) : await Repository.GetFile("def", _rHelper.Job);
                    File.WriteAllBytes(newfile, fileBytes);
                }
                return true;
            }
